...
resource "azurerm_redis_cache" "RedisCache-CKA-dev" {
  ...
  depends_on = [ azurerm_virtual_network.CKA-Vnet, azurerm_subnet.Pvt-Sub2 ]
}
...
resource "azurerm_private_endpoint" "RedisCache-CKA-dev-Pvt-endpoint" {
  ...
  depends_on = [ azurerm_virtual_network.CKA-Vnet, azurerm_subnet.Pvt-Sub2, azurerm_redis_cache.RedisCache-CKA-dev ]
