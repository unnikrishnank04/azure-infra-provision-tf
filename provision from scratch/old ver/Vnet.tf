...
resource "azurerm_subnet" "Pvt-Sub1" {  #Web App Services, ACR, KeyVault
  name                 = var.subnet_name[1]
  resource_group_name  = var.resource_group #azurerm_resource_group.KnowledgeAssist_V2_demo1.name
  virtual_network_name = var.vnet_name #azurerm_virtual_network.CKA-demo1.name
  address_prefixes     = [var.subnet_range[1]]  #64 -->95
  depends_on = [ azurerm_virtual_network.CKA-Vnet, azurerm_subnet.Pub-Sub1-App-Gateway ]
  service_endpoints = [
    "Microsoft.KeyVault"
  ]
  delegation {
...
resource "azurerm_subnet" "Pvt-Sub2" {   #Redis Cache, Postgresql, Storage account
